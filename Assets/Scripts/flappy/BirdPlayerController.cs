...
{
    public static BirdPlayerController Instance;
    ...
    void FixedUpdate()
    {
        if (FlappyBirdController.Instance.currentGameState == FlappyBirdGameStates.Playing)
        {
            // Get the Z rotation of both cubes
            float zRotationCube1 = cube_left.transform.eulerAngles.z;
            float zRotationCube2 = cube_right.transform.eulerAngles.z;

            // Calculate the difference in Z rotation
            float zRotationDifference = zRotationCube2 + zRotationCube1;

            // Clamp the rotation difference to the maximum allowed rotation
            //zRotationDifference = Mathf.Clamp(zRotationDifference, -30f, 30f);

            // Apply the difference to the target object's Z rotation
            cube_center.transform.rotation = Quaternion.Euler(0, 0, zRotationDifference);
            ...
            if (joycons != null && joycons.Count >= 0)
            {
                ...
                if (Grad_force_der > 0.3 + Grad_force_izq)
                { //si un brazo realiza mas fuerza que el otro
                ...
                { //amortigua la inclinación si esta balanceado
                ...
                if (direction.y < 0 && transform.position.y <=-2)
                {
                    direction.y = 0;
                }
